...
namespace Network
{
    public class LobbyManager : MonoBehaviour, INetworkRunnerCallbacks
    {
        ...
        {
            MyNetworkRoot.Instance.Runner.AddCallbacks(this);

            // 3. 광장 접속 (비동기 대기). 
            // 이 순간부터 포톤 서버가 이 클라이언트에게 방 목록을 쏴주기 시작합니다.
            StartGameResult result = await MyNetworkRoot.Instance.Runner.JoinSessionLobby(SessionLobby.Shared);
        ...
        public async void JoinOrCreateRoom(string roomName, int sceneIndex)
        {
            ...
            var sceneManager = MyNetworkRoot.Instance.SceneManagerDefault;
            
            // StartGameArgs는 C++의 접속 설정 Struct라고 보시면 됩니다.
            var result = await MyNetworkRoot.Instance.Runner.StartGame(new StartGameArgs
            {
